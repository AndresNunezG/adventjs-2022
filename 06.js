...
function createCube(size) {
  ...
  for (let i = 0; i < size; i++) {
    const initialSpace = ' '.repeat(size - i - 1)
    const figsA = figA.repeat(i + 1)
    const figsB = figB.repeat(size)
    cube += `${initialSpace}${figsA}${figsB}\n`
  }
  for (let i = 0; i < size; i++) {
    const initialSpace = ' '.repeat(i + 1 - 1)
    const figsC = figC.repeat(size - i)
    const figsD = figD.repeat(size)
    cube += `${initialSpace}${figsC}${figsD}\n`
  }
  return cube.substring(0, cube.length - 1)
}
