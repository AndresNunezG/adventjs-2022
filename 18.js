...
function dryNumber(dry, numbers) {
  ...
  const auxArray = [...Array(numbers + 1).keys()].slice(1)
  return auxArray.filter((_, index) => String(index + 1).includes(dryStr))
}

//SCORE: 200
function dryNumberV2(dry, numbers) {
  const dryStr = new RegExp(String(dry))
  const auxArray = [...Array(numbers + 1).keys()].slice(1)
  return auxArray.filter((_, index) => (
    (index + 1).toString().match(dryStr) != null
  ))
}

console.log(dryNumber(1, 15))
console.log(dryNumber(2, 20))
console.log(dryNumber(0, 32342))
