...
function fitsInOneBox(boxes) {
  ...
  const boxesByVolume = boxesCopy.sort((a, b) => {
    const getVolume = (box) => box.l * box.w * box.h
    const vA = getVolume(a)
    const vB = getVolume(b)
    if (vA > vB) return 1
    if (vA < vB) return -1
    return 0
  })
  for (let i = 0; i < boxesByVolume.length - 1; i++) {
    const boxA = boxesByVolume[i]
    const boxB = boxesByVolume[i + 1]
    if (
      boxA.l >= boxB.l ||
      boxA.w >= boxB.w ||
      boxA.h >= boxB.h
...
console.log(fitsInOneBox(boxesA))
console.log(fitsInOneBox(boxesB))
console.log(fitsInOneBox(boxesC))
console.log(fitsInOneBox(boxesD))
