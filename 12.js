...
function selectSleigh(distance, sleighs) {
  sleighs.forEach(element => (
    element.bttry = 20 - (element.consumption * distance)
  ));
  const filteredSleighs = sleighs
    .filter((sleigh) => (sleigh.bttry >= 0))
    .sort((a, b) => (a.bttry - b.bttry))
  return filteredSleighs && filteredSleighs[0] ? filteredSleighs[0].name : null
}
