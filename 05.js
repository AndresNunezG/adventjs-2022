...
function getMaxGifts(giftsCities, maxGifts, maxCities) {
  ...
  giftsCities.forEach(giftCity => {
    const newCombination = combinations.map(item => {
      let combination = [...item]
      if (combination.length < maxCities) {
        combination.push(giftCity)
      }
      return combination
    })
    combinations = combinations.concat(newCombination)
  });
  ...
  return Math.max(
    ...combinations.map(combination => {
      const total = combination.reduce((acc, item) => acc + item, 0)
      return total > maxGifts ? 0 : total
    })
  )
}
